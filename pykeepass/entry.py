...
class Entry(BaseElement):
    ...
    def history(self):
        if self._element.find("History") is not None:
            return [
                Entry(element=x, kp=self._kp)
                for x in self._element.find("History").findall("Entry")
            ]
        else:
            return []
    ...
    def path(self):
        ...
        if self.is_a_history_entry:
            pentry = Entry(
                element=self._element.getparent().getparent(), kp=self._kp
            ).title
