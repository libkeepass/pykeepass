...
class EntryHistoryTests3(KDBX3Tests):
    ...
    def test_find_history_entries(self):
        ...
        # dito username, assuming if this works, it will also work for all
        # other find_by cases
        ...
        # back in time I had the problem that the first call to save_history()
        # worked but not the second
        for entry in res:
            entry.save_history()
...
class KDBXTests(unittest.TestCase):
    def test_open_save(self):
        ...
        filenames_in = [
            ...
            stream,
            os.path.join(
                base_dir, "test4_aes_uncompressed.kdbx"
            ),  # KDBX v4 AES uncompressed test
        ]
        filenames_out = [
