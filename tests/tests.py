...
class EntryFindTests3(KDBX3Tests):
    ...
    def test_find_entries_by_title(self):
        ...
        results = self.kp.find_entries_by_title(
            "Root_entry", regex=True, flags="i", first=True
        )
    ...
    def test_find_entries_by_username(self):
        ...
        results = self.kp.find_entries_by_username(
            "Foobar_user", regex=True, flags="i", first=True
        )
    ...
    def test_find_entries_by_password(self):
        ...
        self.assertEqual(len(results), 2)
        results = self.kp.find_entries_by_password(
            "Passw0rd", regex=True, flags="i", first=True
        )
    ...
    def test_find_entries_by_notes(self):
        ...
        self.assertEqual(len(results), 2)
        results = self.kp.find_entries_by_notes(
            "Entry notes", regex=True, flags="i", first=True
        )
    ...
    def test_find_entries_by_string(self):
        ...
        self.assertIsInstance(results, Entry)
        self.assertEqual(
            "custom field value", results.get_custom_property("custom_field")
        )
    ...
    def test_find_entries_by_autotype_sequence(self):
        ...
        self.assertEqual(len(results), 1)
        self.assertEqual(
            results[0].autotype_sequence, "{USERNAME}{TAB}{PASSWORD}{ENTER}"
        )
    ...
    def test_find_entries(self):
        ...
        self.assertEqual(len(results), 0)
        results = self.kp.find_entries(
            title="Root_entry", regex=True, flags="i", first=True
        )
        self.assertEqual("root_entry", results.title)
    ...
    def test_add_delete_move_entry(self):
        ...
        self.kp.add_entry(
            subgroup, title="foobar_entry", username="foobar", password="foobar"
        )

        # test adding entry to root which exists in subgroup
        self.kp.add_entry(
            subgroup, title="foobar_entry2", username="foobar", password="foobar"
        )
        self.kp.add_entry(
            self.kp.root_group,
            title="foobar_entry2",
    ...
    def test_expiration_time_tz(self):
        ...
        expiry_time = datetime.now(timezone(offset=timedelta(hours=6))).replace(
            microsecond=0
        )
        self.kp.add_entry(
            self.kp.root_group,
...
class GroupFindTests3(KDBX3Tests):
    ...
    def test_find_groups_by_name(self):
        ...
        results = self.kp.find_groups(
            group=g, name=".*group.*", regex=True, recursive=False
        )
        self.assertEqual(len(results), 1)
    ...
    def test_add_delete_move_group(self):
        ...
        base_group = self.kp.add_group(
            self.kp.root_group, "base_group", notes=notes_text
        )
...
class RecycleBinTests3(KDBX3Tests):
    def test_recyclebincreation(self):
        ...
        entry = self.kp.add_entry(
            self.kp.root_group, "RecycleBinTest1", "login", "password"
        )
    ...
    def test_entry(self):
        entry = self.kp.add_entry(
            self.kp.root_group, "RecycleBinTest2", "login", "password"
        )
        entry_uuid = entry.uuid
        self.kp.trash_entry(entry)

        entries_in_root = self.kp.find_entries(
            uuid=entry_uuid, group=self.kp.root_group, recursive=False
        )
        self.assertEqual(len(entries_in_root), 0)

        entries_in_recyclebin = self.kp.find_entries(
            uuid=entry_uuid, group=self.kp.recyclebin_group, recursive=False
        )
    ...
    def test_group(self):
        ...
        groups_in_root = self.kp.find_groups(
            uuid=group_uuid, group=self.kp.root_group, recursive=False
        )
        self.assertEqual(len(groups_in_root), 0)

        groups_in_recyclebin = self.kp.find_groups(
            uuid=group_uuid, group=self.kp.recyclebin_group, recursive=False
        )
    ...
    def test_recyclebinemptying(self):
        entry = self.kp.add_entry(
            self.kp.root_group, "RecycleBinTest4 Entry", "login", "password"
        )
        self.kp.trash_entry(entry)
...
class EntryTests3(KDBX3Tests):
    ...
    def test_references(self):
        ...
        original_entry_duplicate = self.kp.find_entries(
            title="foobar_entry", first=True
        )
        ...
        self.assertEqual(
            prefixed.deref("username"), "domain\\{}2".format(original_entry.username)
        )
        self.assertEqual(
            prefixed.deref("password"), "A{}BC".format(original_entry.password)
        )
...
class EntryHistoryTests3(KDBX3Tests):
    ...
    def test_find_history_entries(self):
        ...
        e1 = self.kp.add_entry(
            self.kp.root_group, prefix + "title", prefix + "user", prefix + "pass"
        )
...
class CtxManagerTests(unittest.TestCase):
    def test_ctx_manager(self):
        with PyKeePass(
            base_dir / "test4.kdbx", password="password", keyfile=base_dir / "test4.key"
        ) as kp:
...
class KDBXTests(unittest.TestCase):
    ...
    def test_open_save(self):
        ...
        filenames_in = [
            ...
            base_dir
            / "test4_twofish_uncompressed.kdbx",  # KDBX v4 Twofish uncompressed
            base_dir
            / "test4_chacha20_uncompressed.kdbx",  # KDBX v4 ChaCha uncompressed
        ...
        filenames_out = [
            ...
            base_dir
            / "test4_twofish_uncompressed.kdbx.out",  # KDBX v4 Twofish uncompressed
            base_dir
            / "test4_chacha20_uncompressed.kdbx.out",  # KDBX v4 ChaCha uncompressed
            base_dir / "test4_argon2id.kdbx.out",
            base_dir / "test4.kdbx.out",  # KDBX v4 with keyfile file descriptor
            base_dir
            / "test4_blankpass.kdbx.out",  # KDBX v4 with keyfile file descriptor
        ]
        passwords = [
            "password",
            "password",
        ...
        for (
            ...
            kp = PyKeePass(
                filename_in, password, keyfile, transformed_key=transformed_key
            )
            ...
            kp.save(filename_out, transformed_key=transformed_key)
            ...
            kp = PyKeePass(
                filename_out, password, keyfile, transformed_key=transformed_key
            )
    ...
    def test_open_error(self):
        ...
        for database, password, keyfile, error in zip(
            databases, passwords, keyfiles, errors
        ):
            with self.assertRaises(error):
                PyKeePass(base_dir / database, password, base_dir / keyfile)
    ...
    def test_open_no_decrypt(self):
        ...
        enc_algs = ["aes256", "chacha20"]
        versions = [
            (3, 1),
            (4, 0),
        ]
        for database, password, enc_alg, version in zip(
            databases, passwords, enc_algs, versions
        ):
            kp = PyKeePass(os.path.join(base_dir, database), password, decrypt=False)
